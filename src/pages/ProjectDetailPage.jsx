...
import { ArrowLeft, ArrowRight, ExternalLink, Calendar, Clock, Layers } from 'lucide-react';
                        ...
                        <div className="absolute bottom-6 left-6 md:bottom-10 md:left-10">
                            <div className="flex flex-wrap gap-2 mb-3">
                                {project.categories.map((cat, i) => (
                                    <span
                                        key={i}
                                        className="text-xs px-3 py-1 rounded-full bg-white/10 backdrop-blur-sm text-white border border-white/10"
                                    >
                                        {cat}
                                    </span>
                                ))}
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-12 lg:gap-16 mb-16 md:mb-20">
                        ...
                        <AnimatedElement animation="fadeUp">
                            ...
                            <div className="flex flex-wrap gap-3 mb-8">
                                {project.services.map((service, i) => (
                                    <span
                                        key={i}
                                        className="px-4 py-2 rounded-full bg-surface border border-white/10 text-text-secondary text-sm"
                                    >
                                        {service}
                                    </span>
                                ))}
                    ...
                    <div>
                        <AnimatedElement animation="fadeUp" delay={0.15}>
                            <div className="space-y-6 p-6 rounded-2xl bg-surface/50 border border-white/5">
                                <div>
                        ...
                        <StaggerContainer className="grid grid-cols-1 sm:grid-cols-3 gap-6">
                            {project.results.map((result, index) => (
                                <StaggerItem key={index}>
                                    <motion.div
                                        className="p-6 md:p-8 rounded-2xl bg-surface/50 border border-white/5 text-center hover:border-primary/20 transition-colors duration-300"
                                        whileHover={{ y: -4 }}
                                    >
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    {prevProject ? (
                        ...
                        >
                            ...
                            <div>
                                <span className="text-text-muted text-xs uppercase tracking-wider">Önceki Proje</span>
                    ...
                    {nextProject && (
                        ...
                        >
                            <div>
                                <span className="text-text-muted text-xs uppercase tracking-wider">Sonraki Proje</span>
                                <p className="text-white font-medium group-hover:text-primary transition-colors duration-300">
                                    {nextProject.title}
                                </p>
                            </div>
