...
import { AnimatedElement, StaggerContainer, StaggerItem } from '../components/common';
...
export default function ProjectsPage() {
    ...
    const allCategories = useMemo(() => {
        const cats = new Set();
        projectsData.projects.forEach((p) => p.categories.forEach((c) => cats.add(c)));
        return ['Tümü', ...Array.from(cats)];
    }, []);

    const filteredProjects = useMemo(() => {
        if (activeFilter === 'Tümü') return projectsData.projects;
        return projectsData.projects.filter((p) => p.categories.includes(activeFilter));
    }, [activeFilter]);
            ...
            <div className="container-custom">
                ...
                <AnimatedElement animation="fadeUp" className="mb-12 md:mb-16">
                    ...
                    <p className="text-text-secondary text-lg max-w-2xl leading-relaxed">
                        Stratejiden tasarıma, geliştirmeden pazarlamaya — her projemizde markaları öne çıkarıyoruz.
                    </p>
                </AnimatedElement>
                        ...
                        {allCategories.map((cat) => (
                            <button
                                ...
                                className={`px-5 py-2 rounded-full text-sm font-medium transition-all duration-300 border ${activeFilter === cat
                                        ...
                                        : 'bg-transparent text-text-secondary border-white/10 hover:border-primary/40 hover:text-white'
                                    }`}
                            >
                                {cat}
                            </button>
                            ...
                            {filteredProjects.map((project, index) => (
                                ...
                                >
                                    <Link to={`/calismalar/${project.id}`}>
                                        <motion.article
                                            className="group relative rounded-2xl overflow-hidden bg-surface border border-white/5 cursor-pointer h-full"
                                        ...
                                        >
                                            ...
                                            <div className="relative aspect-[16/10] overflow-hidden">
                                                ...
                                                <div className="absolute top-4 right-4">
                                                    <span className="text-xs px-3 py-1 rounded-full bg-white/10 backdrop-blur-sm text-white/80 border border-white/10">
                                                        {project.year}
                                                    </span>
                                                </div>
                ...
                {filteredProjects.length === 0 && (
                    <div className="text-center py-20">
                        <p className="text-text-secondary">Bu kategoride henüz proje bulunmuyor.</p>
                    </div>
                )}
            </div>
        </div>
