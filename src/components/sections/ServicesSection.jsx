                    ...
                    <div className="space-y-0">
                        {servicesData.services.map((service, index) => {
                            ...
                            return (
                                <motion.div
                                    key={service.id}
                                    className={`group cursor-pointer border-b border-white/5 transition-all duration-300 ${isActive ? 'bg-surface/50' : 'hover:bg-surface/30'
                                        }`}
                                ...
                                >
                                    <div className="py-6 px-4">
                                        <div className="flex items-center justify-between">
                                            <div>
                                                <h3
                                                    className={`text-lg md:text-xl font-semibold transition-colors duration-300 ${isActive ? 'text-white' : 'text-text-secondary group-hover:text-white'
                                                        }`}
                                                >
                                                    {service.title}
                                                </h3>
                                                <p
                                                    className={`text-xs tracking-[0.15em] uppercase mt-1 transition-colors duration-300 ${isActive ? 'text-primary' : 'text-text-muted'
                                                        }`}
                                                >
                                                    {service.subtitle}
                                                </p>
                                            ...
                                            <motion.div
                                                className={`transition-all duration-300 ${isActive ? 'opacity-100' : 'opacity-0 group-hover:opacity-50'
                                                    }`}
                            ...
                            <div className="flex justify-center gap-2 mt-6">
                                {servicesData.services.map((_, index) => (
                                    <button
                                        key={index}
                                        onMouseEnter={() => setActiveService(index)}
                                        onClick={() => handleServiceClick(servicesData.services[index].id)}
                                        className={`w-2 h-2 rounded-full transition-all duration-300 ${activeService === index
                                            ? 'bg-primary w-6'
